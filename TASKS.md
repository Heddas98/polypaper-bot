...
- [ ] **T4.6** — Simülatör↔gerçek PnL parity smoke (Single Fee Oracle identity × 3 trade class). T4.5 ile beraber yapılabilir — empirical değerlerle backtest sweep + sim vs live PnL delta < 5% kabul.
...
- [ ] **T4.10** *(yeni 2026-04-24)* — `executions.regime_at_entry` write path. Phase 79+ migration kolonu ekledi ama hiç populate edilmiyor (T4.5 calibration: regime breakdown sadece "unknown" 1082/1082). `RegimeDetector.current_regime()` + order placement sırasında write → regime × slippage × PnL correlation analizi mümkün olur.
- [ ] **T6.3-B** *(yeni 2026-04-24)* — `/diagnose` UI halt ghost. `/risk` "No halt" → doğru, `/diagnose` "Halt: Active" → yanıltıcı UI. T6.3 ghost class doktrini paralelinde, 5 ghost sınıfı setine ek (drift_monitor, autopilot, kelly_sizing, market_recorder, halt-display). `core/engine.py` halt source vs `diagnose_handler.py` reader cross-check, fix ya da deprecate. Mainnet blocker DEĞİL.
