...
- [ ] **T7.6** T1.4 Faz 3 — bare `except Exception` daraltma (MED-LOW, gerçek 113 blok × 26 dosya) — risk: MED, bağımlılık: **T1.4 Faz 1 ✅ (2026-04-20)** + T7.1-T7.5 ✅ (2026-04-21) *(T1.4'ten devredildi; Epic 7 closure 2026-04-21'de müstakil mini-epic olarak ayrıldı)*
  ...
  - **Aşama A — Easy wins (~42 blok, 16 küçük dosya):** adaptive trackers (becker×2, micro = 12), telemetry (changelog, bg_task, keepalive, decision_explainer, observability = 14), signal utils (ev_tracker, signal_fusion, kelly, strategy_selector, experiment_runner, intent_parser, whale_flow = 9), engine_support (2). Risk LOW, per-dosya atomic commit.
    - **Progress (2026-04-22):** 5/16 dosya ✅ — commit chain `d4f0b7c` (observability shadow-module fix BONUS), `0e3d1e5` (kelly), `ed47ca6` (strategy_selector), `348a6dd` (whale_flow + Optional drop + test strengthen), `23a31c6` (engine_support × 2 blok). 6 blok narrow, 1 CRITICAL shadow-bug kurtarıldı (Phase 48 cid sistem yeniden canlı), 1 unused import, 1 test güçlendirme. Regression: 229 pass + 2 skip (SIGNAL_W_WHALE=0.10 kurulumla).
    - **Kalan 11 dosya:** ev_tracker, signal_fusion, experiment_runner, intent_parser, decision_explainer, keepalive, bg_task, becker_weight_tracker, micro_weight_tracker, changelog, becker_rolling_recal.
