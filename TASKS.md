...
- [ ] **B3 (opt)** — F-string log eager-eval audit (info/warn level, hot path dışı). Low severity; Epic 10 log performans geçişinde birlikte bakılabilir.
- [ ] **B6 (opt)** — `safe_create_task` ref capture güçlendirmesi. `_BG_TASK_REGISTRY` global zaten var ama debuggability için explicit `add_done_callback` eklenebilir. Epic 8 T8.1 sırasında engine.py tarafında async lifecycle incelenince tekrar bakılacak.
- [ ] **B7 (opt)** — Agent FP doğrulandı ama `risk_manager.per_asset_limits` dataclass initialization pattern dokümante edilebilir (docstring note). Çok düşük öncelik.
- [ ] **Post-audit doc follow-up** — `core/stats_utils.py` yeni modülü `docs/ARCHITECTURE.md`'ye ekle (adaptive tracker'ların shared utility'si olarak). Epic 11 öncesi arch senkronizasyonu için.
...
- [ ] **T8.1** T1.4 Faz 2 + T7.6 Aşama C — `ai_brain.py` + `engine_signals.py` + `engine.py` + `auto_optimizer.py` bare except daraltma (**146 blok HIGH risk**) — bağımlılık: **T1.4 Faz 1 ✅ (2026-04-20)** + T7.6 Aşama A+B ✅ (2026-04-22)
  - **Kapsam güncel (2026-04-22 Aşama C devir kararı):** `ai_brain.py` 47 + `engine_signals.py` 42 + `engine.py` 35 + `auto_optimizer.py` 22 = **146 blok HIGH risk, 4 dosya**. T8.3 auto_optimizer audit'i T8.1 ile birlikte yürütülecek — exception tipi bağlamı paylaşıyor (CLOB/HTTP/DB/telegram zinciri + PnL pause + startup health-check). Ayrıca küçük noqa kalan blokları: `bg_task` 3 (kasıtlı guard-of-guard) + `strategy_suggester` 4 (kasıtlı singleton). Bunlar T8.1 içinde gerekçe annotation'ı eksikse eklenecek, silinmeyecek.
  - **Metodoloji:** Faz 1 + Aşama B ile birebir aynı workflow — (1) dosya başına grep `except Exception` + context; (2) blok tablosu (line/fonksiyon/gerçek hata tipi/narrow önerisi + dead code tespiti); (3) kullanıcı onayı; (4) commit-per-concern (dead code sil → narrow tipler → debug upgrade → noqa annotation); (5) py_compile + AST handler count + grep doğrulama. Per-dosya atomic commit zinciri (Aşama A+B pattern).
  - **Alt-bağımlılık:** T8.3 `_startup_health_check` protected strategy denetimi bu görevle birlikte yapılacak (auto_optimizer.py zaten inceleniyor olacak). T1.3'te silinen 16 ghost modülün ai_brain.py/engine_signals.py/engine.py'de kalan orphan pattern'i varsa (capital_allocator/whale_signal/cascade_detector gibi) engine_settlement/engine_fills'deki gibi dead code olarak silinecek.
  - **Post-audit takip (T7.6 wide audit bekleyen):** B6 (`safe_create_task` ref capture audit — engine.py async lifecycle incelenince), stats_utils.py ARCHITECTURE.md entry (T8.1 kapanışında Epic 11 doc senkronu).
  - **Beklenen çıktı:** 146 → ~35-45 catch-all (gerekçeli # noqa: BLE001) + ~100 narrow, 4 dosya py_compile + AST temiz, core/ genelinde toplam generic bare <25.
- [ ] **T8.2** `ANTHROPIC_API_KEY` kullanım yerleri, rate-limit guard var mı — risk: MED
- [ ] **T8.3** ✅ **T8.1'e dahil edildi (2026-04-22):** `auto_optimizer.py::_startup_health_check` — PNL_PAUSE_THRESHOLD protected strategy'yi yanlışlıkla pause ediyor mu? — risk: HIGH. auto_optimizer bare except narrowing ile aynı oturumda yapılacak, ayrı task olarak kalmıyor.
