...
- [ ] **T7.6** T1.4 Faz 3 — bare `except Exception` daraltma (MED-LOW, gerçek 113 blok × 26 dosya) — risk: MED, bağımlılık: **T1.4 Faz 1 ✅ (2026-04-20)** + T7.1-T7.5 ✅ (2026-04-21) *(T1.4'ten devredildi; Epic 7 closure 2026-04-21'de müstakil mini-epic olarak ayrıldı)*
  ...
  - **Aşama C — HIGH risk (146 blok, 4 dosya):** `ai_brain.py` (47), `auto_optimizer.py` (22), `engine.py` (35), `engine_signals.py` (42). Optuna tuning + AI decision path + 14-gate signal pipeline + engine orchestration. T8.1 ile overlap (Faz 2 123 bloğun büyük kısmı burada). Ayrı oturum + daha sıkı test. **Mainnet sonrası ayrı PR (SKIP kararı).**
