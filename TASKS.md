...
- [x] **T5.3** ✅ 2026-04-21 — `pending_reserved` audit. **Bulgu**: Tek hesap sitesi (`engine_signals.py:1232`), `engine.py`'de paralel hesap yok (docstring referansı var sadece). Aktif race YOK (evals `for s in strats:` sequential). **Fix B**: `_compute_pending_reserved(wallet_id)` helper extract (DRY). **Fix A**: locked append scope'u içine defensive re-check eklendi, yeni skip reason `RESERVED_OVERFLOW` — future-proof (paralel eval introduced olursa overdraw catch). **Test C**: `tests/unit/test_pending_reserved.py` 11 test (multi-wallet isolation + overflow predicate). 11/11 pass. — commit: `<TBD>`
...
- [ ] **T5.5** SQLite WAL autocheckpoint (database.py:49, 5000) — uzun süreli yazım kilidi ihtimalini doğrula — risk: LOW
- [ ] **T5.6** WS subscription cap (`MAX_WS_TOKENS=200`) taşıyor — `WS token cap reached (200/200), skipping N new` WARN (kaynak: `data/websocket_client.py:84`, Phase 79b guardrail) — risk: MED
  - *Teşhis*: Hata değil, tasarım gereği warn. Paper scan token havuzu 200+ büyüyor; eklenmek istenen yeni tokenlar **sessizce atlanıyor** → o marketlerde live fiyat yok → stale/no-price karar riski.
  - *Live etki*: Shadow/Live 3 strateji × ~10 market × 2 token ≈ 60 sub → sınıra çarpmıyor; etki sadece paper cycle'da.
  - *Kök sebep hipotezi*: `prune_stale_tokens()` (line 92-106) çağrı frekansı yetersiz ve Polymarket WS unsubscribe API'si yok → `_subscribed` kümesi monotonic büyüyor. Reconnect flush (T5.4) kardeşi.
  - *Yapılacak*: (a) `prune_stale_tokens` çağrı sitelerini listele ve frekans + kapsamını doğrula, (b) Scan cycle'da "aktif/önceliksiz" token ayrımı + top-K cap, (c) Protected/shadow-live tokenları için "must-subscribe" öncelik listesi, (d) `MAX_WS_TOKENS` varsayılanını ölçülü artır (400-500), (e) WARN oranı telemetri (cycle/dk).
