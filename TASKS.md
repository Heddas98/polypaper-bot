...
- [ ] **T11.8-B** *(yeni 2026-04-24)* — Advisory zone bare-except narrow (data/ + telegram_bot/ + db/, 366 violation). T11.8 core/ strict kapandı (`2088d6e`); advisory dirler raporlama-only. Aşamalı batch (data/* → telegram_bot/jobs/* → telegram_bot/handlers/* → db/*).
  ...
  - [x] **Aşama D (db/)** ✅ **2026-04-24** — 4 dosya × 21 → **0** bare-except (tam narrow, hiç noqa gerekmedi). `migration_phase79.py` (1→0: migrate runner → (sqlite3.Error, OSError)), `migrations.py` (5→0: _get_current_version/apply/run/grandfather tümü aiosqlite.Error), `database.py` (6→0: 2× atomic retry aiosqlite.Error + 2× stats aiosqlite.Error + 2× settings aiosqlite.Error), `ro_connect.py` (9→0: 2× _try_connect inner sqlite3.Error/aiosqlite.Error + 2× immutable fallback + 2× sidecar copy OSError/SameFileError + 2× stage-3 tmp-copy mixed + close sqlite3.Error + unlink OSError). db/ violation: 21 → 0.
