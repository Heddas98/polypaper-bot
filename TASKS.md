...
- [ ] **T7.6** T1.4 Faz 3 — bare `except Exception` daraltma (MED-LOW, gerçek 113 blok × 26 dosya) — risk: MED, bağımlılık: **T1.4 Faz 1 ✅ (2026-04-20)** + T7.1-T7.5 ✅ (2026-04-21) *(T1.4'ten devredildi; Epic 7 closure 2026-04-21'de müstakil mini-epic olarak ayrıldı)*
  ...
  - **Aşama A — Easy wins (~42 blok, 16 küçük dosya):** adaptive trackers (becker×2, micro = 12), telemetry (changelog, bg_task, keepalive, decision_explainer, observability = 14), signal utils (ev_tracker, signal_fusion, kelly, strategy_selector, experiment_runner, intent_parser, whale_flow = 9), engine_support (2). Risk LOW, per-dosya atomic commit.
    - **Progress (2026-04-22):** 6/16 dosya ✅ — commit chain `d4f0b7c` (observability shadow-module fix BONUS), `0e3d1e5` (kelly), `ed47ca6` (strategy_selector), `348a6dd` (whale_flow + Optional drop + test strengthen), `23a31c6` (engine_support × 2 blok), `651ff53` (ev_tracker × 2 blok + Optional drop). **8 blok narrow** + 1 CRITICAL shadow-bug kurtarıldı (Phase 48 cid sistem yeniden canlı) + 2 unused import + 1 test güçlendirme.
    - **Ara-audit (2026-04-22):** 6 dosyada 0 bare + 0 generic Exception. Observability end-to-end: CorrelationFilter canlı UUID basıyor. Full suite 479 pass + 6 skip + 10 fail, 10/10 fail pre-existing (6 sandbox env, 2 test-order pollution, 2 ENV default drift); değiştirdiğim dosyalara dokunan fail yok.
    - **Kalan 10 dosya (29 blok, AST doğrulandı):** signal_fusion (2), experiment_runner (1), intent_parser (1), decision_explainer (3), keepalive (3), bg_task (3 + 1 BaseException noqa), becker_weight_tracker (4), micro_weight_tracker (4), changelog (4), becker_rolling_recal (4).
  - **Aşama B — Medium (~52 blok, 7 orta dosya + Faz 1 leftover):** trade_journal (8), strategy_suggester (7), engine_monitor (6), strategy_lifecycle (6), autopilot (5), trade_memory (5) + Faz 1 leftover re-review (engine_settlement 8, engine_fills 2, live_trader 7 — muhtemelen gerekçeli catch-all, narrow sonrası kalan). Risk MED.
  - **Aşama C — HIGH risk (22 blok, 1 dosya):** `core/auto_optimizer.py` — Optuna tuning + PnL pause guard + scheduled runs. T8.3 `_startup_health_check` protected strategy güvenliği ile overlap. Ayrı oturum + daha sıkı test.
