...
def test_streak_cooldown() -> None:
    ...
    rm.state.last_loss_ts = (datetime.now(UTC) - timedelta(hours=7)).isoformat()
    v2 = rm.check_trade(1.0, "BTC-test", 1000.0)
    check(rm.state.consecutive_losses == 0, "cooldown auto-reset streak to 0")
    check(v2.approved, f"Gate 7 passes after cooldown (got: {v2.reason})")
...
def test_live_trader_derive_failure() -> None:
    ...
    check(
        "failed" in detail.lower() or "not installed" in detail.lower() or "err" in detail.lower(),
...
def test_schema_has_is_maker_migration() -> None:
    ...
    check(
        "ALTER TABLE executions ADD COLUMN is_maker" in src, "is_maker ALTER on executions present"
    )
    check(
        "ALTER TABLE live_trades ADD COLUMN is_maker" in src,
...
def test_strategy_status_case_insensitive() -> None:
    ...
    check(
        "def _missing_" in src and "StrategyStatus" in src,
...
def test_becker_replay_harness_unit() -> None:
    ...
    s = r.summarize()
    check(s["trades"] == 2 and s["win_rate"] == 50.0, "mixed summary counts + wr correct")

    # Strategy registry integrity
    check("threshold_70" in STRATEGY_REGISTRY, "threshold_70 registered")
    check("contra_70" in STRATEGY_REGISTRY, "contra_70 registered")

    # threshold_strategy behavior
    strat = threshold_strategy(
        threshold=0.70, min_seconds_left=60, max_seconds_left=600, stake_usd=1.0
    )
    # Below threshold → no decision
    ctx = ReplayContext(
        slug="s", now_ts=0, yes_price=0.5, seconds_to_end=300, open_position=None, resolved_yes=1.0
    )
    check(strat(ctx) is None, "below threshold = no decision")
    # At threshold, within window → open YES
    ctx = ReplayContext(
        slug="s", now_ts=0, yes_price=0.75, seconds_to_end=300, open_position=None, resolved_yes=1.0
    )
    d = strat(ctx)
    check(
        d is not None and d.action == "open" and d.side == "YES",
...
def test_promote_handler_html_escaped() -> None:
    ...
    check("from telegram_bot.templates.safe_html import" in src, "promote.py imports safe_html")
    check("esc(label)" in src or "esc(row['label']" in src, "promote.py wraps label with esc()")
    check(
        "esc_code(sid)" in src or "esc_code(prefix)" in src,
...
def test_becker_latency_and_partial_fill() -> None:
    ...
    check("max(0.01, min(1.0, float(fill_fraction)))" in src, "fill_fraction clamped to (0,1]")
...
def test_keepalive_env_gated() -> None:
    ...
    check(
        "KeepAlive(engine, db) if _keepalive_enabled else None" in src,
