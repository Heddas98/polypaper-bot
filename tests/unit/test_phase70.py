...
class TestCalibrationSurface:
    ...
    def test_surface_lookup_out_of_range(self):
        ...
        surface.cells[(0.50, 2)] = SurfaceCell(
            price_bin=0.50, time_bin=2, delta=0.05, n_trades=100)
        surface.built = True
        # Price too low
    ...
    def test_surface_lookup_with_cell(self):
        ...
        surface = CalibrationSurface()
        surface.cells[(0.50, 2)] = SurfaceCell(
            price_bin=0.50, time_bin=2, delta=0.05,
            n_trades=200, confidence=1.0)
    ...
    def test_surface_delta_convenience(self):
        from calibration.surface_2d import surface_delta, CalibrationSurface
        # No surface, no fallback
        r = surface_delta(None, 0.50)
        assert r.source == "no_surface"

    def test_surface_boost(self):
        from calibration.surface_2d import surface_boost, SurfaceResult
        r = SurfaceResult(boost=0.05)
        assert surface_boost(r) == 0.05

    def test_format_surface_telegram(self):
        from calibration.surface_2d import (
            format_surface_telegram, CalibrationSurface)
        surface = CalibrationSurface()
...
class TestMCI:
    ...
    def test_mci_sparse_surface(self):
        ...
        surface.built = True
        surface.cells[(0.50, 2)] = SurfaceCell(
            price_bin=0.50, time_bin=2, delta=0.02, n_trades=50)
    ...
    def test_mci_good_surface(self):
        ...
        for i in range(15):
            ...
            surface.cells[(pb, 2)] = SurfaceCell(
                price_bin=pb, time_bin=2, delta=delta,
                n_trades=200, confidence=1.0)
            if mirror_pb != pb:
                surface.cells[(mirror_pb, 2)] = SurfaceCell(
                    price_bin=mirror_pb, time_bin=2, delta=-delta,
                    n_trades=200, confidence=1.0)
    ...
    def test_mci_format_telegram(self):
        from calibration.coherence import format_mci_telegram, MCIResult
...
class TestPennyContract:
    ...
    def test_penny_low_zone_signal(self):
        from core.strategy_plugins import PennyContractStrategy, MarketSnapshot
        s = PennyContractStrategy()
        snap = MarketSnapshot(
            up_odds=0.03, down_odds=0.97,
            spread=0.02, minutes_remaining=10.0)
    ...
    def test_penny_tight_spread_skip(self):
        ...
        snap = MarketSnapshot(
            up_odds=0.03, down_odds=0.97,
            spread=0.005, minutes_remaining=10.0)
        result = s.evaluate(snap)
        assert result.should_trade is False
    ...
    def test_penny_too_close_to_close(self):
        ...
        snap = MarketSnapshot(
            up_odds=0.03, down_odds=0.97,
            spread=0.02, minutes_remaining=0.5)
        result = s.evaluate(snap)
        assert result.should_trade is False
