...
async def test_data_status_full_panel():
    ...
    sync.execute("INSERT INTO external_prices VALUES (?,?,?,?)",
    ...
    assert any(kw in text for kw in
               ("Backtest", "Veri", "DB", "external_prices", "ob_deltas"))
    await db_conn.close()
...
async def test_ref_audit_with_data():
    ...
    rows = [
        # Healthy BTC: ~0.1 bps
        (now - 3600000, "btc-1", "0xt", "btc-1", "BTC", "5m",
         100000.0, None, 100001.0, None, 0.10, None, "UP", "ok", iso),
        (now - 7200000, "btc-2", "0xt", "btc-2", "BTC", "5m",
         101000.0, None, 101000.5, None, 0.05, None, "DOWN", "ok", iso),
        # Bad ETH: 10 bps systemic bias
        (now - 1800000, "eth-1", "0xt", "eth-1", "ETH", "15m",
         3500.0, None, 3503.5, None, 10.0, None, "UP", "ok", iso),
        (now - 5400000, "eth-2", "0xt", "eth-2", "ETH", "15m",
         3505.0, None, 3508.5, None, 9.99, None, "DOWN", "ok", iso),
    ]
    sync.executemany(
        "INSERT INTO reference_price_audit VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        rows)
    sync.commit(); sync.close()
    db_conn = await aiosqlite.connect(db_file.name)
    db = SimpleNamespace(conn=db_conn)
...
async def test_recon_running(monkeypatch):
    ...
    task._mismatches = [
        {"ts": "2026-05-09T12:00:00Z", "delta_usd": 1.5,
         "onchain_pusd": 9.5, "db_pusd": 11.0},
        {"ts": "2026-05-09T13:00:00Z", "delta_usd": -2.0,
         "onchain_pusd": 8.0, "db_pusd": 10.0},
    ]
