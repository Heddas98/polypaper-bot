...
> **2026-05-19 paper PnL drift ✅ KAPALI** (`20536ba`): "1417 trade / +$355" = doğrulanmamış 2026-04-30 master-plan snapshot'ı; tüm trade artefaktları 2026-05-08'de başlıyor (mainnet öncesi DB reset, `schema_version` #13). Reset'ten sağ çıkan `wallets.balance` **$9.966,99** ($10k altı) → +$355 çürük. Gerçek: `executions` 251 trade / **-$54,28** / WR %45,8, tüm stratejiler net negatif. Düzeltme: `_get_paper_summary` → `wallets.balance`, sahte `PAPER_BUDGET_DISPLAY` kaldırıldı.
...
> **2026-05-19 market likidite filtresi ✅** (`447c5a3`): "no match" KÖK NEDEN — `has_liquidity` sahteydi (`polymarket_client` midpoint üzerine zorla True → `engine_signals.py:373` NO_LIQ gate etkisiz). Polymarket docs MCP ile denetlendi. A: sahte override kaldırıldı. B: `_compute_liquidity` — `get_orderbook` iki-yanlı derinlik $2 eşiği (`MARKET_DEPTH_CHECK`/`MIN_BOOK_DEPTH_USD` env). C: `_market_tradeable` — `enableOrderBook`/`acceptingOrders` discovery filtresi. 14 test, 2555 PASS. Ek: `uma_dispute.should_block_new_position` production'a bağlı değil (backlog).
> **2026-05-19 fiyat-hareketi (delta) verisi ✅** (`5f506e6`): Heddas direktifi — market açılış→kapanış delta + son 5/10/24s ort.|hareket|. `candle_collector.compute_price_deltas` saf fonksiyon, veri `candles_ext` (zaten toplu, yeni API yok). Market BUY onay ekranı tam blok + Piyasa Tara kompakt inline + PAPER MODE'a "📡 Piyasa Tara" butonu (mod-bağımsız). `test_price_delta.py` 12 test, 2423 PASS.
> **2026-05-19 fiyat-delta revize ✅** (`4220d16`): Heddas — güncel (canlı) delta + okunabilir blok. `compute_price_deltas` `current_*` alanları; `_fetch_price_deltas` `external_feed` canlı spot → `live_delta` (pencere açılışından beri). `_price_delta_block` 4 etiketli net satır (▶️ canlı / ⏹ önceki / 📊 ort. hareket / 🧭 yön+yorum), kriptik kısaltmalar kaldırıldı. 14 test, 2414 PASS.
