...
- **`/live` redesign (2026-05-18, Heddas direktifi "trade istasyonu")**: Faz 1 kokpit ✅ (`e39042f` bilgi-yoğun panel: risk-bar, Binance momentum, regime, streak; `f99b0b0` streak PAPER etiketi). Budget Reset butonu ✅ (`1b2f506`, 2-tık onaylı). Faz 2A veri katmanı ✅: `live_trades` boş bug — `execute_market_order` INSERT eksikti (`9803037` fix), `pusd_allowance` ♾️ UI (`d574924`). **Açık: manuel trade settle eşleştirme** — güvenilir kaynak yok (`market_events` boş, Polymarket `closed_positions` belirsiz semantik `cashPnl=null`); reconciliation backlog. Faz 2B paneller (Piyasa/Risk/Guards) + Faz 3 backlog.
- **Veri bulgusu**: `engine.risk` streak = PAPER (sadece `engine_settlement.py:633`'ten beslenir, live değil). Polymarket `closed_positions.realized_pnl` data-api raw — semantik belirsiz, "kazandın/kaybettin" için güvenilmez; gerçek = Polymarket UI / pUSD bakiye.
