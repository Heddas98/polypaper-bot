...
async def edit_strategy_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sl = (
        f"{s.stop_loss_odds}"
        if s.stop_loss_odds
        else ("Off" if not s.stop_loss_percent else f"{s.stop_loss_percent*100:.0f}%")
    )
    tp = (
        f"{s.take_profit_odds}"
        if s.take_profit_odds
        else ("Off" if not s.take_profit_percent else f"{s.take_profit_percent*100:.0f}%")
    )
    ...
    kb = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    f"📛 İsim: {s.label or '—'}", callback_data=f"se_label_{s.id}"
                ),
                InlineKeyboardButton(f"💰 ${s.trade_amount}", callback_data=f"se_amount_{s.id}"),
            ],
            [
                InlineKeyboardButton(
                    f"🎯 Tetik: {s.odds_threshold}", callback_data=f"se_trigger_{s.id}"
                ),
            ...
            [
                ...
                InlineKeyboardButton(
                    f"📐 PD:{s.price_difference or 'Off'}", callback_data=f"se_pdiff_{s.id}"
                ),
            ],
            [
            ...
            [
                InlineKeyboardButton(
                    f"📏 EMA:{'✅' if s.ma_filter_enabled else '❌'}",
                    callback_data=f"se_ema_{s.id}",
                ),
                InlineKeyboardButton(
                    f"🔢 Max:{s.max_executions_per_event or '∞'}", callback_data=f"se_maxe_{s.id}"
                ),
            ],
            [
                InlineKeyboardButton(
...
async def handle_edit_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ok:
        await update.message.reply_text(
            f"✅ <b>Güncellendi!</b>\n<b>{field}</b> → <code>{esc(value)}</code>", parse_mode="HTML"
        )
    else:
