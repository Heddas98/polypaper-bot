...
async def _send(message, db, user, page: int = 0, edit: bool = False):
    ...
    header = (
        f"🎰 <b>{wallet.label} Strategies</b>  "
        f"(▶{active} / ⏸{stopped} / Σ{total})"
    )
        ...
        if pages > 1:
            ...
            if page > 0:
                nav.append(InlineKeyboardButton(
                    "⬅️ Önceki", callback_data=f"strats_page_{page-1}"))
            nav.append(InlineKeyboardButton(
                f"{page+1}/{pages}", callback_data="strats_noop"))
            if page < pages - 1:
                nav.append(InlineKeyboardButton(
                    "Sonraki ➡️", callback_data=f"strats_page_{page+1}"))
            btns.append(nav)
...
async def start_all_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"▶ <b>{n} strateji başlatıldı.</b>", parse_mode="HTML")
...
async def stop_all_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for s in await db.get_strategies_by_user(user.id, wallet.id):
        if s.status == StrategyStatus.ACTIVE:
            ...
            n += 1
    await update.message.reply_text(
        f"⏸ <b>{n} strateji durduruldu.</b>", parse_mode="HTML")


async def edit_strategy_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Phase 19.5: Strategy detail with INLINE EDIT BUTTONS."""
    q = update.callback_query
    sid = q.data.replace("edit_strat_", "")
    db: Database = context.bot_data["db"]
    s = await db.get_strategy(sid)
    if not s:
        return await q.answer("Strateji bulunamadı")
    await q.answer()

    name = s.label or s.auto_label()
    te = {"fusion": "🔬", "contrarian": "🔄", "sniper": "🎯",
          "momentum": "📈", "scalper": "⚡", "martingale": "🎰", "highthreshold": "🏔️", "flashcrash": "💥", "streak": "🔄"}.get(s.strategy_type, "🔬")
    status = "✅" if s.status == StrategyStatus.ACTIVE else "⚫"
    sl = f"{s.stop_loss_odds}" if s.stop_loss_odds else ("Off" if not s.stop_loss_percent else f"{s.stop_loss_percent*100:.0f}%")
    tp = f"{s.take_profit_odds}" if s.take_profit_odds else ("Off" if not s.take_profit_percent else f"{s.take_profit_percent*100:.0f}%")

    # Phase 49 P0-05: escape user-provided label + strategy_type
    from telegram_bot.templates.safe_html import esc, esc_code
    text = (
        f"{te} <b>{esc(name)}</b> {esc(status)}\n\n"
        f"📛 İsim: <b>{esc(s.label or '—')}</b>\n"
        f"💰 Tutar: <b>${s.trade_amount}</b>\n"
        f"🎯 Tetik: <b>{s.odds_threshold}</b>\n"
        f"↕️ Yön: <b>{esc(s.direction.value)}</b>\n"
        f"📊 Tip: <b>{esc(s.strategy_type)}</b>\n"
        f"📉 SL: <b>{esc(sl)}</b> | 📈 TP: <b>{esc(tp)}</b>\n"
        f"⏱ Pencere: <b>{s.minutes_after_start}m → {s.minutes_before_end}m</b>\n"
        f"📐 Price Diff: <b>{esc(s.price_difference or 'Off')}%</b>\n"
        f"📏 EMA: <b>{'✅' if s.ma_filter_enabled else '❌'}</b>\n"
        f"🔢 Max Exec: <b>{esc(s.max_executions_per_event or '∞')}</b>\n"
        f"ID: <code>{esc_code(s.id[:12])}</code>\n\n"
        f"👇 Düzenlemek için butona tıklayın:"
    )

    stopped = s.status == StrategyStatus.STOPPED
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"📛 İsim: {s.label or '—'}", callback_data=f"se_label_{s.id}"),
         InlineKeyboardButton(f"💰 ${s.trade_amount}", callback_data=f"se_amount_{s.id}")],
        [InlineKeyboardButton(f"🎯 Tetik: {s.odds_threshold}", callback_data=f"se_trigger_{s.id}"),
         InlineKeyboardButton(f"↕️ {s.direction.value}", callback_data=f"se_dir_{s.id}")],
        [InlineKeyboardButton(f"📊 {s.strategy_type}", callback_data=f"se_type_{s.id}"),
         InlineKeyboardButton(f"📐 PD:{s.price_difference or 'Off'}", callback_data=f"se_pdiff_{s.id}")],
        [InlineKeyboardButton(f"📉 SL:{sl}", callback_data=f"se_sl_{s.id}"),
         InlineKeyboardButton(f"📈 TP:{tp}", callback_data=f"se_tp_{s.id}")],
        [InlineKeyboardButton(f"📏 EMA:{'✅' if s.ma_filter_enabled else '❌'}", callback_data=f"se_ema_{s.id}"),
         InlineKeyboardButton(f"🔢 Max:{s.max_executions_per_event or '∞'}", callback_data=f"se_maxe_{s.id}")],
        [InlineKeyboardButton("▶ Start" if stopped else "⏸ Stop",
                              callback_data=f"{'start' if stopped else 'stop'}_strat_{s.id}"),
         InlineKeyboardButton("🗑 Sil", callback_data=f"delete_strat_{s.id}")],
        [InlineKeyboardButton("⬅️ Stratejiler", callback_data="show_strategies")],
    ])
    await q.message.reply_text(text, parse_mode="HTML", reply_markup=kb)
...
EDIT_FIELDS = {
    "se_label": ("label", "📛 Yeni isim girin:", str),
    "se_amount": ("trade_amount", "💰 Yeni tutar girin ($):", float),
    "se_trigger": ("odds_threshold", "🎯 Yeni tetik eşiği girin (0.01-0.99):", float),
    "se_pdiff": ("price_difference", "📐 Fiyat farkı % girin (0=kapalı):", float),
    "se_sl": ("stop_loss_odds", "📉 Stop Loss odds girin (0=kapalı):", float),
    "se_tp": ("take_profit_odds", "📈 Take Profit odds delta girin (0=kapalı):", float),
    "se_maxe": ("max_executions_per_event", "🔢 Max işlem/event girin:", int),
}


async def strategy_field_edit_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Phase 19.5: Handles all se_* callbacks — asks user for new value."""
    q = update.callback_query
    data = q.data

    # Parse: se_FIELD_STRATEGYID
    for prefix, (field, prompt, _) in EDIT_FIELDS.items():
        if data.startswith(prefix + "_"):
            sid = data[len(prefix) + 1:]
            context.user_data["editing"] = {"sid": sid, "field": field, "type": prefix}
            await q.answer()
            await q.message.reply_text(prompt)
            return

    # Special: EMA toggle (no text input needed)
    if data.startswith("se_ema_"):
        sid = data[7:]
        db: Database = context.bot_data["db"]
        s = await db.get_strategy(sid)
        if s:
            new_val = 0 if s.ma_filter_enabled else 1
            await db.update_strategy_field(sid, "ma_filter_enabled", new_val)
            emoji = "✅" if new_val else "❌"
            await q.answer(f"EMA: {emoji}")
            await q.message.reply_text(f"✅ EMA filtresi: <b>{emoji}</b>", parse_mode="HTML")
        return

    # Special: Direction cycle (up → down → any → up)
    if data.startswith("se_dir_"):
        sid = data[7:]
        db: Database = context.bot_data["db"]
        s = await db.get_strategy(sid)
        if s:
            cycle = {"up": "down", "down": "any", "any": "up"}
            new_dir = cycle.get(s.direction.value, "any")
            await db.update_strategy_field(sid, "direction", new_dir)
            await q.answer(f"Yön: {new_dir}")
            await q.message.reply_text(f"✅ Yön: <b>{new_dir}</b>", parse_mode="HTML")
        return

    # Special: Type cycle
    if data.startswith("se_type_"):
        sid = data[8:]
        db: Database = context.bot_data["db"]
        s = await db.get_strategy(sid)
        if s:
            types = ["fusion", "contrarian", "sniper", "momentum", "scalper", "martingale"]
            idx = types.index(s.strategy_type) if s.strategy_type in types else 0
            new_type = types[(idx + 1) % len(types)]
            await db.update_strategy_field(sid, "strategy_type", new_type)
            await q.answer(f"Tip: {new_type}")
            await q.message.reply_text(f"✅ Strateji tipi: <b>{new_type}</b>", parse_mode="HTML")
        return
...
async def handle_edit_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Phase 19.5: Catches text input when user is editing a field."""
    ...
    cast = str
    for _, (f, _, t) in EDIT_FIELDS.items():
        if f == field:
            cast = t
            break

    try:
        if text.lower() in ("off", "kapalı", "0", "none") and cast in (float, int):
            value = None
        else:
            value = cast(text)
    except (ValueError, TypeError):
        await update.message.reply_text(f"❌ Geçersiz değer: '{esc(text)}'")
        return

    db: Database = context.bot_data["db"]
    ok = await db.update_strategy_field(sid, field, value)
    context.user_data.pop("editing", None)

    if ok:
        await update.message.reply_text(
            f"✅ <b>Güncellendi!</b>\n<b>{field}</b> → <code>{esc(value)}</code>",
            parse_mode="HTML")
    else:
        await update.message.reply_text(f"❌ Güncellenemedi.")
...
def _quick_strategy_label(stype: str, asset: Asset, tf: Timeframe,
                          direction: Direction, odds: float) -> str:
...
async def quick_strategy_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 4:
        ...
        return await update.message.reply_text(
            _quick_strategy_usage_text(), parse_mode="HTML")
    ...
    s = Strategy(user_id=user.id, wallet_id=wallet.id,
                 label=auto_lbl,
                 asset=asset, timeframe=tf, trade_amount=amt,
                 odds_threshold=odds, direction=direction,
                 strategy_type=stype)
...
async def quick_strategy_wizard_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("qs_o_"):
        ...
        return await _edit_or_reply(q,
            ...
            f"📊 <b>Type seç:</b>\n"
            f"<i>momentum→UP, contrarian→DOWN, diğerleri→ANY</i>", kb)
...
async def edit_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(matched) > 1:
        return await update.message.reply_text(f"⚠️ '{prefix}' birden fazla eşleşiyor. Daha uzun ID kullanın.")

    s = matched[0]

    # Type conversion
    float_fields = {"trade_amount", "odds_threshold", "price_difference",
                    "stop_loss_percent", "stop_loss_odds", "take_profit_percent",
                    "take_profit_odds", "min_volatility", "max_entry_slippage",
...
async def clone_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(matched) > 1:
        return await update.message.reply_text(f"⚠️ Birden fazla eşleşme. Daha uzun ID kullanın.")
    ...
    from db.models import Strategy, StrategyStatus, Asset, Timeframe, Direction
    clone = Strategy(
        user_id=user.id, wallet_id=wallet.id,
        label=f"KLON_{s.label or s.auto_label()}",
        asset=s.asset, timeframe=s.timeframe, direction=s.direction,
        trade_amount=s.trade_amount, odds_threshold=s.odds_threshold,
        price_difference=s.price_difference,
        minutes_after_start=s.minutes_after_start,
        minutes_before_end=s.minutes_before_end,
        stop_loss_percent=s.stop_loss_percent, stop_loss_odds=s.stop_loss_odds,
        take_profit_percent=s.take_profit_percent, take_profit_odds=s.take_profit_odds,
...
OPTIMIZED_PRESETS = [
    {
        ...
        "bet_from": 0.5, "bet_to": 0.25,
        "sl_odds": 0.40, "tp_odds": 0.15,
        "max_exec": 2, "max_losses": 1,
        "slippage": None, "ema": True, "volatility": 0.3,
        "stype": "fusion",
    },
    {
        ...
        "amount": 1.0, "trigger": 0.30, "price_diff": 5.0,
        "bet_from": 1.0, "bet_to": 0.5,
        "sl_odds": None, "tp_odds": 0.20,
        "max_exec": 1, "max_losses": 1,
        "slippage": None, "ema": False, "volatility": 0,
        "stype": "contrarian",
    },
    {
        ...
        "asset": "BTC", "timeframe": "5m", "direction": "any",
        "amount": 1.0, "trigger": 0.40, "price_diff": 3.0,
        "bet_from": 0.5, "bet_to": 0.5,
        "sl_odds": None, "tp_odds": 0.20,
        "max_exec": 1, "max_losses": 1,
        "slippage": None, "ema": False, "volatility": 0,
        "stype": "contrarian",
    ...
    },
    {
        ...
        "asset": "BTC", "timeframe": "5m", "direction": "down",
        "amount": 1.0, "trigger": 0.50, "price_diff": 2.0,
        "bet_from": 0.5, "bet_to": 0.25,
        "sl_odds": 0.40, "tp_odds": 0.15,
        "max_exec": 2, "max_losses": 1,
    ...
    },
    {
        ...
        "asset": "BTC", "timeframe": "5m", "direction": "any",
        "amount": 1.0, "trigger": 0.90, "price_diff": 3.0,
        "bet_from": 0.5, "bet_to": 0.25,
        "sl_odds": None, "tp_odds": None,
        "max_exec": 1, "max_losses": 1,
        "slippage": None, "ema": True, "volatility": 0.5,
        "stype": "sniper",
    },
    {
    ...
    },
    {
        ...
        "asset": "BTC", "timeframe": "5m", "direction": "any",
        "amount": 2.0, "trigger": 0.80, "price_diff": 0,
        "bet_from": 0.5, "bet_to": 0.5,
        "sl_odds": None, "tp_odds": None,
        "max_exec": 1, "max_losses": 3,
        "slippage": None, "ema": False, "volatility": 0,
    ...
    },
    {
        ...
        "asset": "BTC", "timeframe": "5m", "direction": "any",
        "amount": 1.0, "trigger": 0.40, "price_diff": 0,
        "bet_from": 0.5, "bet_to": 0.5,
        "sl_odds": None, "tp_odds": None,
        "max_exec": 1, "max_losses": 3,
        "slippage": None, "ema": False, "volatility": 0,
    ...
    },
    {
        ...
        "asset": "BTC", "timeframe": "5m", "direction": "any",
        "amount": 1.0, "trigger": 0.50, "price_diff": 0,
        "bet_from": 0.5, "bet_to": 0.5,
        "sl_odds": None, "tp_odds": None,
        "max_exec": 1, "max_losses": 3,
        "slippage": None, "ema": False, "volatility": 0,
...
async def optimize_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, p in enumerate(OPTIMIZED_PRESETS):
        text += f"\n<b>{i+1}. {p['name']}</b>\n{p['desc']}\n"
        flags = []
        if p.get("ema"): flags.append("EMA")
        if p.get("volatility"): flags.append(f"Vol={p['volatility']}%")
        if p.get("slippage"): flags.append(f"Slip={p['slippage']}")
        text += f"   {p['asset']}/{p['timeframe']} thr={p['trigger']} [{p['stype']}] {' '.join(flags)}\n"
        buttons.append([InlineKeyboardButton(
            f"{i+1}. {p['name']}", callback_data=f"opt_deploy_{i}")])
...
async def kelly_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    strats = await db.conn.execute_fetchall(
        ...
        WHERE s.status='active' AND s.user_id=? GROUP BY s.id ORDER BY wins DESC""", (user.id,))
    text = f"🎯 <b>Quarter Kelly Sizing</b>\n💰 Bakiye: <b>${balance:.2f}</b>\n━━━━━━━━━━━━━━━━━━━━━\n"
    for s in strats:
...
async def analyze_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
        ...
        if m in ("brain", "auto", "otonom"): mode = "brain"
    ...
    try:
        if mode == "brain":
            ...
            if result:
                await update.message.reply_text(f"🧠 <b>Brain Cycle Sonucu</b>\n\n{result}", parse_mode="HTML")
            else:
            ...
            if not response:
                return await update.message.reply_text("⚠️ Analiz sonuc uretmedi.", parse_mode="HTML")
            ...
            if parsed and parsed.get("actions"):
                ...
                text = (f"📋 <b>Onerilen Aksiyonlar ({len(actions)} adet)</b>\n"
                        f"━━━━━━━━━━━━━━━━━━━━━\n"
                ...
                keyboard = InlineKeyboardMarkup([
                    [InlineKeyboardButton("🧠 Brain Cycle Calistir", callback_data="analyze_brain")]
                ])
                await update.message.reply_text(
...
async def analyze_optimize_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Loading indicator
    await update.message.reply_text("⏳ <b>Otonom Brain Cycle Baslatiliyor</b>...", parse_mode="HTML")
...
async def micro_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if clo is None:
        lines.append("ℹ️ Chainlink oracle disabled "
                     "(<code>CHAINLINK_ORACLE_ENABLED=true</code> ile aç).")
...
async def recorder_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not recorder:
        return await update.message.reply_text(
            "⚠️ Market Recorder aktif degil.", parse_mode="HTML")
