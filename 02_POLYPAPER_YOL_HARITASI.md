...
- **Bulgu:** Polymarket Up/Down binary'leri için "official resolution price" Polymarket tarafından numeric değil binary outcome ("Up"/"Down") olarak yayınlanır. **⚠️ DÜZELTME 2026-05-19: resolution kaynağı = Chainlink BTC/USD data stream — Binance DEĞİL (Gamma API doğruladı). Bu satırdaki "Resolution oracle = Binance spot kline" ifadesi ve "Binance klines ground-truth" audit metodolojisi yanlış zemine oturuyor; `scripts/audit_reference_price.py` revize edilmeli.**
