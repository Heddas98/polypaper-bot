...
DEAD_VARS = {
    ...
    "CASCADE_ENABLED",
    "CASCADE_MAX_HOLD_SEC",
    "CASCADE_OVERSHOOT_PCT",
    "CASCADE_PRICE_THRESHOLD",
    "CASCADE_VOLUME_MULT",
    "CASCADE_WINDOW_SEC",
    # Capital allocator — silinmiş
    "CAPITAL_ALLOCATOR_ENABLED",
    "CAPITAL_BASE_ALLOCATION",
    "CAPITAL_MAX_PER_STRATEGY",
    "CAPITAL_MIN_PER_STRATEGY",
    "CAPITAL_PERFORMANCE_WEIGHT",
    "CAPITAL_REBALANCE_INTERVAL",
    "CAPITAL_TOTAL_BUDGET",
    # DB Retention — kullanılmıyor (default OK)
    "DB_RETENTION_CANDLES_EXT_DAYS",
    "DB_RETENTION_CANDLES_POLY_DAYS",
    "DB_RETENTION_OB_SNAPSHOTS_DAYS",
    "DB_RETENTION_OB_TRADES_DAYS",
    "DB_RETENTION_ODDS_HISTORY_DAYS",
    # Event waves — silinmiş
    "EVENT_WAVES_ENABLED",
    "EVENT_WAVES_MIN_QUALITY",
...
def main():
    ...
    if args.apply:
        backup = env_path.with_suffix(
            env_path.suffix + f".pre_cleanup_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S')}"
        )
